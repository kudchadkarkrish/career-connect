...
export const StudentCard = ({ student, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-700 shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between overflow-hidden group">
      <div className="p-5">
        {/* Header: Name & Status */}
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="min-w-0">
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 transition-colors truncate">
              {student.name}
            </h3>
            <div className="flex items-center space-x-2 mt-0.5">
              ...
              <span className="flex items-center text-xs text-slate-500 dark:text-slate-400">
                <Hash className="w-3 h-3 mr-0.5 text-slate-400 dark:text-slate-500" />
                {student.rollNo}
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className="flex items-center text-xs text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 px-2.5 py-1.5 rounded-lg flex-1">
            <GraduationCap className="w-3.5 h-3.5 mr-1.5 text-indigo-500 shrink-0" />
            <span className="font-semibold text-slate-900 dark:text-slate-100">CGPA {student.cgpa.toFixed(2)}</span>
          </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {Array.isArray(student.skills) && student.skills.length > 0 ? (
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500 italic">No skills listed</span>
            )}
            {student.skills && student.skills.length > 4 && (
              <span className="inline-flex items-center text-[11px] bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded">
        ...
        {student.status === 'Placed' && student.placedDetails && (
          <div className="pt-2.5 border-t border-slate-100 dark:border-slate-800">
            <div className="flex items-start text-xs text-slate-600 dark:text-slate-400 space-x-1.5">
              <Building2 className="w-3.5 h-3.5 text-emerald-600 shrink-0 mt-0.5" />
              <div className="min-w-0">
                <span className="font-semibold text-slate-900 dark:text-slate-100">{student.placedDetails.companyName}</span>
      ...
      <div className="bg-slate-50/70 dark:bg-slate-800/60 border-t border-slate-100 dark:border-slate-800 px-5 py-2.5 flex items-center justify-end space-x-2">
        <button
          type="button"
          onClick={() => onEdit(student)}
          className="inline-flex items-center text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-indigo-600 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
        >
