              ...
              return (
                <tr key={student.id} className="hover:bg-slate-50/70 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-slate-400 truncate max-w-[140px]">{student.email}</div>
                  </td>

                  {/* Roll No */}
                  <td className="px-4 py-3">
                    <span className="text-xs font-mono text-slate-600">{student.rollNo}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`text-sm font-bold ${
                      student.cgpa >= 8.5 ? 'text-emerald-700' :
                      student.cgpa >= 7.0 ? 'text-slate-800' :
                      'text-amber-700'
                    }`}>
                  ...
                  <td className="px-4 py-3 hidden lg:table-cell">
                    <div className="flex flex-wrap gap-1">
                      {(student.skills || []).slice(0, 3).map((skill, idx) => (
                        <span key={idx} className="text-[11px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded font-medium">
                          {skill}
                        </span>
                      ))}
                      {student.skills && student.skills.length > 3 && (
                        <span className="text-[11px] text-slate-400 px-1">+{student.skills.length - 3}</span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end space-x-1.5">
                      <button
                        ...
                        className="inline-flex items-center text-xs font-medium text-slate-600 hover:text-indigo-600 bg-white hover:bg-indigo-50 border border-slate-200 px-2 py-1 rounded-lg transition-colors cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center text-xs font-medium text-rose-600 hover:text-rose-700 bg-white hover:bg-rose-50 border border-slate-200 hover:border-rose-200 px-2 py-1 rounded-lg transition-colors cursor-pointer"
                      >
